 ...
 *
 */
public class LeftOuterJoinTest extends MobiusJob {
	...
	public void test()
	{
		...
		{			
			...
			File trueAnswer = new File(this.getClass().getResource("/com/ebay/erl/mobius/core/left.outerjoin.true.answer").toURI());
			
			String[] args	= new String[]{input1.getAbsolutePath(), input2.getAbsolutePath(), output.getAbsolutePath()};
			
			int exitCode = MobiusJobRunner.run(new LeftOuterJoinTest(), args);
			if( exitCode==0 )
			{
