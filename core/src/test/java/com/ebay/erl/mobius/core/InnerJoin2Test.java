 ...
 *
 */
public class InnerJoin2Test extends MobiusJob
{

	private static final long serialVersionUID = -4316665261736737004L;
	...
	public void test()
	{
		...
		{			
			...
			File trueAnswer = new File(this.getClass().getResource("/com/ebay/erl/mobius/core/innerjoin2.true.answer").toURI());
			
			String[] args	= new String[]{input1.getAbsolutePath(), input2.getAbsolutePath(), output.getAbsolutePath()};
			
			int exitCode = MobiusJobRunner.run(new InnerJoin2Test(), args);
			if( exitCode==0 )
			{
